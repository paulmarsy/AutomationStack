function Write-DeploymentUpdate {
    ...
    $udp = if ($null -eq $CurrentContext) {[string]::Empty} else {$CurrentContext.Get('UDP')}
    $Host.UI.RawUI.WindowTitle = 'AutomationStack{0} {1}/{2}' -f $udp, $StageNumber, $TotalDeploymentStages
