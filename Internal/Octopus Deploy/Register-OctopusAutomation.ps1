function Register-OctopusAutomation {
    ...
    Remove-AzureRmAutomationCredential -ResourceGroupName $CurrentContext.Get('ResourceGroup') -AutomationAccountName $CurrentContext.Get('AutomationAccountName') -Name OctopusDeployServiceAccount -ErrorAction Ignore
    ...
    New-AzureRmAutomationCredential -ResourceGroupName $CurrentContext.Get('ResourceGroup') -AutomationAccountName $CurrentContext.Get('AutomationAccountName') -Name OctopusDeployServiceAccount -Value $octopusDeployServiceAccount | Out-Host
    
    Write-Host "Importing Octopus Deploy DSC Configuration..."

    $CurrentContext.Set('OctopusVMName', 'OctopusVM')
    $CurrentContext.Set('OctopusConnectionString', 'Server=tcp:#{SqlServerName}.database.windows.net,1433;Initial Catalog=OctopusDeploy;Persist Security Info=False;User ID=#{SqlServerUsername};Password=#{SqlServerPassword};MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Connection Timeout=30;')
