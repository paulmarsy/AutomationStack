...
filter Write-Log {
    ...
    Add-Content -Path $localLogFile -Value $entry
    Add-Content -Path $remoteLogFile -Value $entry
}
...
try {
    ...
    "{0}Waiting for Local Configuration Manager{0}" -f ("-"*40) | Write-Log
